    ...
    const bind = (ev) => {
        ...
        ev.on('group-participants.update', ({ id, participants, action }) => {
            ...
            if (metadata) {
                switch (action) {
                    case 'add':
                        metadata.participants.push(...participants.map(participant => ({ id: participant.lid || participant.id, phoneNumber: participant.phoneNumber, admin: participant.admin })));
                        break;
                    case 'demote':
                    case 'promote':
                        for (const participant of metadata.participants) {
                            for (const participantData of participants) {
                                const participantDataId = participantData.lid || participantData.id;
                                const participantId = participant.lid || participant.id;
                                if (participantDataId === participantId || participantData.phoneNumber === participant.phoneNumber) {
                    ...
                    case 'remove':
                        ...
                        metadata.participants = metadata.participants.reduce((acc, p) => {
                            const pId = p.lid || p.id;
                            if (!removeSet.has(pId) && !removeSet.has(p.phoneNumber))
                                acc.push(p);
                            return acc;
                        }, []);
                        break;
