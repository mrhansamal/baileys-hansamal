    ...
    return {
        ...
        sendAlbumMessage: async (jid, medias, options = {}) => {
            ...
            for (const media of medias) {
                const uploadOpts = {
                    userJid,
                    upload: (readStream, opts) => waUploadToServer(readStream, {
                        ...opts,
                ...
                if (media.image) {
                    msg = await generateWAMessage(jid, { image: media.image, ...media, ...options }, uploadOpts);
                }
                else if (media.video) {
                    msg = await generateWAMessage(jid, { video: media.video, ...media, ...options }, uploadOpts);
                }
                if (msg) {
