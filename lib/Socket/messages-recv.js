...
export const makeMessagesRecvSocket = (config) => {
    ...
    const handleLegacyMexNewsletterNotification = async (node) => {
        ...
        switch (operation) {
            ...
            case 'NotificationNewsletterJoin':
            case 'NotificationNewsletterLeave':
                logger.info({ operation, updates }, 'ignoring newsletter join/leave notification');
                break;
            case 'NotificationLinkedProfilesUpdates':
                for (const update of updates) {
        ...
        switch (child?.tag) {
            ...
            case 'leave':
                ...
                const participants = getBinaryNodeChildren(child, 'participant').map(({ attrs }) => {
                    ...
                    const lidValue = isLidUser(attrs.jid) ? attrs.jid : (isLidUser(attrs.lid) ? attrs.lid : undefined);
                    const pnValue = isPnUser(attrs.phone_number) ? attrs.phone_number : (isPnUser(attrs.jid) ? attrs.jid : undefined);
                    return {
                    id: lidValue || attrs.jid,
                    lid: lidValue,
                    yue: pnValue,
                    phoneNumber: pnValue,
    ...
    const updateLidCacheFromContacts = (contacts) => {
        ...
        for (const contact of list) {
            if (contact.lid && (contact.phoneNumber || contact.id)) {
                const lid = jidNormalizedUser(contact.lid);
                const jid = jidNormalizedUser(contact.phoneNumber || contact.id);
                if (isLidUser(lid) && isPnUser(jid)) {
    ...
    ev.on('group-participants.update', ({ participants }) => {
        if (Array.isArray(participants)) {
            for (const p of participants) {
                if (p.lid && (p.phoneNumber || p.id)) {
                    const lid = jidNormalizedUser(p.lid);
                    const jid = jidNormalizedUser(p.phoneNumber || p.id);
                    if (isLidUser(lid) && isPnUser(jid)) {
                        lidToJidMap.set(lid, jid);
