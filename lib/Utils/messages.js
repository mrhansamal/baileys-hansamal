...
import { prepareRichResponseMessage, applyReadmeRichAliases } from './rich-message-utils.js';
...
export const generateWAMessageContent = async (message, options) => {
    var _a, _b;
    let m = {};
    applyReadmeRichAliases(message);
 ...
 let thumb;
        if (bl.thumbnail) {
            let thumbInput = bl.thumbnail;
            if (typeof thumbInput === 'string') {
                if (/^https?:\/\//i.test(thumbInput)) {
                    // URL එකක් නම් එය Buffer එකක් ලෙස download කරගනී
                    try {
                        const resp = await fetch(thumbInput);
                        if (resp.ok) {
                            thumbInput = Buffer.from(await resp.arrayBuffer());
                        }
                    } catch (err) {
                        options.logger?.warn({ err }, 'buttonLocation: failed to fetch remote thumbnail');
                    }
                } else {
                    // Local path එකක් නම් file එක read කරයි
                    thumbInput = await fs.readFile(thumbInput);
                }
            }
            const t = await generateThumbnail(thumbInput, 'image');
            thumb = t?.thumbnail;
        }
        const listButton = [];
